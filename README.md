...
### 最近更新
**2026.08.21** - **通知过滤与响应式优化、文件查看下载优化与 CLI 文档增强 (v1.1.27)**：新增全局与任务级的通知通道过滤并深度优化中小屏幕响应式适配；优化编辑器内二进制与图片文件的查看和下载机制，解决预览卡顿与强制弹窗体验问题；补充完善了 CLI 命令参数与脚本元数据头部注释格式规范；修复前端 TS 编译报错问题。  
