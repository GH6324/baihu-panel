...
> 💡 **最新版本**：[v1.1.30](https://github.com/engigu/baihu-panel/releases/tag/v1.1.30) (2026.09.11)

* **标签关联资源穿透与快捷跳转**：支持点击标签关联计数弹窗查看绑定的定时任务与环境变量明细，支持一键快捷跳转。
* **标签输入法回车字符丢失修复**：修复前端标签输入框在回车确认时输入法末尾字符丢失的问题。
* **任务结束时序与实时日志竞态修复**：优化任务执行结束时的日志落库与状态时序，修复超短任务高频并发下的实时日志显示问题。
* **调度引擎性能调优与安全漏洞升级**：优化 Cron 注册阶段的数据库查询性能；升级 `golang.org/x/crypto` 至 `v0.55.0` 修复安全漏洞。
