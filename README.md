...
### 最近更新
**2026.09.08** - **Windows 原生 ConPTY 伪终端重构、hosts 自动化域名映射、托盘 Modern UI 升级与仓库代理支持 (v1.1.29)**：伪终端底层重构为 `conpty` 官方依赖库，解决托盘/后台模式下终端卡死；Windows 安装包静默且幂等地自动写入 `127.0.0.1 baihu.local` hosts 映射；系统托盘引入 Windows 11/10 原生沉浸式深色 Modern 菜单；支持仓库同步配置 HTTP 代理（#164）；首页控制台日志支持按今日筛选（#23）；修复 Web 终端手动重连提示错误与站点配置默认值回退机制。  
