...
截止20260922，系统核心蓝图已全面拼齐，从声明式 YAML 的全流程编排定义，到底层任务调度，再到多端消息推送，正式构建起完整的自动化生命周期闭环。
...
> 💡 **最新版本**：[v1.2.0](https://github.com/engigu/baihu-panel/releases/tag/v1.2.0) (2026.09.22)

* **全新声明式应用与官方应用市场**：全新上线 `app.yaml` 规范与应用市场（AppStore），支持生态应用一键秒级安装、场景模式选配、环境变量契约表单及 Master-Child 任务编排解耦架构。
* **定时任务启用状态筛选**：定时任务管理列表全新支持按“全部 / 已启用 / 已禁用”状态一键筛选。
* **2FA 两步验证备份恢复失效修复**：允许用户模型凭证完整导出序列化，彻底修复从备份恢复后因缺失 OTP 密钥导致无法登录的问题 ([#169](https://github.com/engigu/baihu-panel/issues/169))。
* **跨平台路径归一化与安全升级**：优化 Windows 平台脚本路径占位符与命令解析；升级依赖消除 Dependabot 安全漏洞。

<details>
<summary>📜 <b>点击展开查看近期历史版本日志</b></summary>

- **2026.09.11 (v1.1.30)** - 标签关联资源穿透与跳转、输入法回车字符丢失修复、调度引擎稳定性调优与安全升级。
- **2026.09.08 (v1.1.29)** - Windows 原生 ConPTY 伪终端重构、hosts 自动化映射、托盘 Modern UI 升级与仓库代理支持。
- **2026.09.04 (v1.1.28)** - IDE 底部 Terminal 控制台、运行环境持久化、任务配置批量更新、ConPTY 重叠修复与安全升级。
- **2026.08.21 (v1.1.27)** - 通知过滤、小屏展示优化、移动端平滑滚动与 CLI 文档增强。
- **2026.08.17 (v1.1.26)** - 通知内容统一与 Options SDK、日志 SSE 状态帧协议与 Bug 修复。
- **2026.08.10 (v1.1.25)** - 企业微信推送、备份恢复调度自动加载与 4 项安全漏洞修复。
- **2026.07.31 (v1.1.24)** - 标签管理系统、Monaco 编辑器多语言语法高亮与登录两步验证 (OTP/2FA)。
- **2026.07.23 (v1.1.23)** - Windows GUI 安装包与后台系统托盘守护程序 (`baihu-tray.exe`)。
- **2026.07.20 (v1.1.22)** - Windows 平台深度适配、网页终端 `Ctrl+C` 终结与 Linux PTY 崩溃修复。
- **2026.07.13 (v1.1.21)** - ZSTD 日志压缩、依赖自动补全交互式终端与列表视图联动排序。
- **2026.06.28 (v1.1.20)** - 节点互联架构升级（变量同步与 OpenConnect 协议）与终端 OOM 边界防护。
