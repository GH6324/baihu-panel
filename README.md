...
> 💡 **最新版本**：[v1.1.29](https://github.com/engigu/baihu-panel/releases/tag/v1.1.29) (2026.09.08)

* **Windows 原生 ConPTY 伪终端重构**：底层重构为官方 `conpty` 依赖库，解决托盘与后台模式下终端卡死/黑屏问题。
* **Windows 安装包 hosts 自动化**：安装时静默且幂等地自动写入 `127.0.0.1 baihu.local`，卸载静默清理；托盘默认一键打开。
* **Windows 托盘 Modern 沉浸 UI**：引入 Windows 11/10 原生 Modern 深色菜单样式，支持双击托盘图标快速打开控制台。
* **仓库同步代理与首页筛选**：仓库同步任务全新支持配置 HTTP 代理 (#164)；首页控制台日志支持按“今日”快速筛选 (#23)。
