...
async function fetchLogs() {
    ...
    try {
        const res = await api.appLogs.list({
            category: LOG_CATEGORY.FILTER_LOG,
        ...
        toast.error(e.message || '获取过滤日志失败')
    } finally {
        loading.value = false
    }
}
...
async function handleClear() {
    try {
        await api.appLogs.clear(LOG_CATEGORY.FILTER_LOG)
        toast.success('清空成功')
        currentPage.value = 1
        fetchLogs()
    } catch (e: any) {
...
// 实时更新：当有新过滤日志产生且用户在第一页时刷新
useEventBus([LOG_EVENTS.ADDED], (payload) => {
    if (payload && payload.category === LOG_CATEGORY.FILTER_LOG) {
...
<template>
    <div class="space-y-4">
        <BaihuDialog v-model:open="showClearConfirm" title="清空过滤日志确认">
            <div class="text-sm text-muted-foreground leading-relaxed">
                此操作将永久清空当前分类下的所有过滤日志记录，操作后无法恢复。确认要继续吗？
            </div>
            <template #footer>
                ...
                <div v-for="(log, index) in logs" :key="`small-${log.id}`"
                    ...
                    <div class="space-y-1.5 text-xs text-muted-foreground mb-1 px-1">
                        <div class="flex items-center gap-3">
                            <span class="w-8 shrink-0 font-medium opacity-70">规则名:</span>
                            <span class="text-foreground bg-muted/40 px-1.5 py-0.5 rounded text-[10px]">{{ log.error_msg || '-' }}</span>
                        </div>
                        <div class="flex items-start gap-3">
                            <span class="w-8 shrink-0 font-medium mt-0.5 opacity-70">详情:</span>
                ...
                <div v-for="log in logs" :key="`medium-${log.id}`"
                    ...
                    <div class="w-60 shrink-0 flex items-center gap-3 min-w-0 font-medium text-sm">
                        ...
                        <span class="truncate" :title="log.title">
                            <span v-if="log.error_msg" class="mr-1 text-muted-foreground opacity-60">[{{ log.error_msg }}]</span>{{ log.title }}
                        </span>
                    </div>
                ...
                <div class="flex items-center gap-2">
                    <Badge v-if="selectedLog" variant="outline" :class="[
                        ...
                        <div class="flex items-center gap-1 uppercase tracking-tighter">
                            ...
                            <span>{{ selectedLog.level === LOG_LEVEL.WARNING ? '静默拦截' : '内容替换' }}</span>
                        </div>
                    </Badge>
            ...
            <div class="space-y-6">
                <div class="space-y-4">
                    <div class="p-4 rounded-xl bg-muted/20 border border-border/10 space-y-2">
                        <p class="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">匹配与过滤详情</p>
                        <div v-if="detailDialogProps.content" class="text-sm leading-relaxed text-foreground/80 break-all whitespace-pre-wrap max-h-[40vh] overflow-y-auto custom-scrollbar font-mono bg-background/50 p-3 rounded-lg border border-border/40">
