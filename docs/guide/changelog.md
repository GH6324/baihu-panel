...
### 2026.07.13 - 计划任务排序优化与弹窗易用性提升
- **计划任务排序 (New)**：任务列表（大屏及中屏布局）支持点击表头“名称”、“执行时间”（下次执行时间）和“状态”进行排序；后端接口支持 `sort_by` 和 `order` 字段与置顶任务（`pin_type DESC`）优先排序。
- **小屏排序支持**：在小屏/移动端布局下，于筛选栏新增了“排序规则”下拉菜单，支持选择“创建时间”、“名称”、“下次执行”、“状态”等规则，与大屏逻辑无缝统一。
- **全局 ESC 关闭弹窗**：在通用组件 `DialogContent` 内部新增全局 Escape 键盘监听以优雅关闭最顶层 Dialog 弹窗，防止输入框聚焦或终端、Monaco 组件阻断 ESC 退出；自定义日志弹窗同步集成 Escape 事件监听。
- **体验与样式优化**：将“状态”列宽度由 `w-8` 扩大至 `w-14` 以消除排序图标加入引发的折行和表头挤压；去除 Dialog 自动聚焦时产生的窗口边缘白色高亮聚焦线。
- **视图管理联动**：过滤视图现已能够将排序状态（`sort_by` 与 `order`）联动保存并于再次应用时自动还原。
