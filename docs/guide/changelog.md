...
### 2026.09.16 - 声明式 App 存储模型与清理逻辑重构、应用热度统计集成及规范架构优化 (v1.1.31)
- **声明式应用 (App) 存储模型与契约清洗 (Refactor)**：
  - 彻底清洗 App 与受控 Task 节点的数据关联模型，严格厘清主应用节点（`type = 'app'`, `source_id = 'app:' + manifestID`）与受控子任务（`type = 'task'`, `source_id = masterTask.ID`）的数据存储与外键关联规范；
  - 彻底废弃所有兼容性模糊匹配与冗余查询逻辑，所有应用物理资源清理与属性获取统一封装为 `masterTask.GetManifestID()` 与 `app.CleanAppTaskAndData()` 标准函数。
- **应用卸载与受控任务物理级联清理 (Fix & Enhancement)**：
  - 在任务管理列表侧与应用服务侧全面对齐应用物理卸载逻辑，支持在删除 `type = 'app'` 节点时精准级联物理清理受控子任务、推送绑定、标签/环境变量映射以及磁盘本地代码目录 (`data/scripts/apps/:manifestID`)；
  - 在任务删除提示弹窗中显式增加对于应用级物理卸载的影响范围提示。
- **应用市场实时热度统计展示 (Feature)**：
  - 前端应用市场视图全新集成上报统计 API（`https://baihu-appstore-stats.qwapi.eu.org/stats`），在市场头部实时展示平台累计浏览量 (PV) 与应用部署总数，并在各个 App 卡片底栏展示对应应用的真实安装热度；
  - 隐藏应用调度配置编辑过程中的匿名遥测选项，仅在应用市场安装时保留该开关，保障系统调度的纯粹与隐私安全。
- **声明式应用规范文档与 AppStore 仓库重写 (Docs)**：
  - 重写 `docs/guide/app-spec.md` 规范说明，直接关联引导至白虎官方应用商店开源仓库 [engigu/baihu-appstore](https://github.com/engigu/baihu-appstore)；
  - 同步更新白虎面板 App 数据存储与关联关系设计规范。

### 2026.09.11 - 标签关联资源穿透与跳转、输入法体验修复、调度引擎稳定性调优与安全升级 (v1.1.30)
- **标签关联资源穿透与快捷跳转 (Feature)**：
  - 优化“标签管理”页面，支持点击关联资源计数弹窗查看该标签绑定的所有定时任务与环境变量明细列表；
  - 弹窗内支持直接点击资源名称一键跳转至对应的任务管理或环境变量页面进行定位查看与快捷编辑。
- **标签输入组件字符丢失修复 (Fix)**：
  - 修复前端 `TagInput.vue` 组件在按下回车确认添加标签时，输入法缓冲区末尾字符意外丢失的输入体验问题。
- **任务执行结束时序与日志落库优化 (Fix & Perf)**：
  - 优化任务执行结束时的日志压缩与状态入库时序，彻底修复超短任务及高频并发下实时日志流与落库状态存在竞态导致前端显示不同步的隐患；
  - 优化仓库同步任务在 Cron 注册阶段的数据库查询开销，提升调度器整体性能并适度放宽内部通信超时时间。
- **安全依赖升级 (Security)**：
  - 升级 `golang.org/x/crypto` 至 `v0.55.0`，彻底修复已知安全隐患（[#165](https://github.com/engigu/baihu-panel/issues/165)）。

### 2026.09.08 - Windows 原生 ConPTY 伪终端重构、hosts 自动化域名映射、托盘 Modern UI 升级与仓库代理支持 (v1.1.29)
- **Windows 原生 ConPTY 伪终端重构 (New)**：伪终端底层重构为 `github.com/ActiveState/termtest/conpty` 官方库，彻底解决了 Windows 托盘与无控制台模式下管道被占用导致终端卡在“连接中”或黑屏的问题；优化了 `pwsh.exe` 启动参数。
